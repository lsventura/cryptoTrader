...
class Strategy:
    ...
    def manage_position(self, current_position, current_price, new_signal):
        ...
        Normaliza chaves de posição para evitar KeyError em 'amount'
        """
        if not current_position:
            return "OPEN_NEW"
        
        # Normaliza a chave de quantidade (pode ser 'amount', 'contracts', 'qty', etc)
        amount = current_position.get('amount') or current_position.get('contracts') or current_position.get('qty') or 0
        if amount == 0:
            return "OPEN_NEW"

        pos_side = str(current_position.get('side', '')).lower() # 'long' ou 'short'
