...
def calculate_quantity(symbol, investment):
    ...
    return round(quantity, 8)  # Ajustar precisão conforme necessário

# Função para monitorar stop loss e take profit em futuros
def monitor_futures_trade(symbol, quantity, entry_price, stop_loss, take_profit):
    while True:
        ticker = client.futures_symbol_ticker(symbol=symbol)
        ...
        if current_price >= take_profit:
            print(f"Take Profit atingido: {current_price}. Encerrando posição...")
        ...
        if current_price <= stop_loss:
            print(f"Stop Loss atingido: {current_price}. Encerrando posição...")
            client.futures_create_order(
                symbol=symbol,
                side=SIDE_SELL,
                type=ORDER_TYPE_MARKET,
...
def auto_trade_futures():
    active_trades = {}
    investment_amount = 5  # Valor a ser investido em dólares por par
    ...
    while True:
        ...
        for result in best_results:
            ...
            if last_signal == 1:
                ...
                entry_price = float(client.futures_symbol_ticker(symbol=pair)['price'])
                ...
                monitor_futures_trade(pair, quantity, entry_price, stop_loss, take_profit)
